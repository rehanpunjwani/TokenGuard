...
class StorageConfig(BaseModel):
    db_path: str = "~/.local/share/recall-py/recall-py.db"
...
def _env_config_overlay() -> dict[str, Any]:
    ...
    o: dict[str, Any] = {}
    if v := os.environ.get("RECALL_PY_OLLAMA_BASE_URL", "").strip():
        o.setdefault("ollama", {})["base_url"] = v
    if v := os.environ.get("RECALL_PY_DB_PATH", "").strip():
        o.setdefault("storage", {})["db_path"] = v
    if v := os.environ.get("RECALL_PY_API_LISTEN_HOST", "").strip():
        o.setdefault("api", {})["listen_host"] = v
    if v := os.environ.get("RECALL_PY_API_LISTEN_PORT", "").strip():
        try:
    ...
    def load(cls, path: Path | None = None) -> AppSettings:
        ...
        for candidate in _default_yaml_candidates():
            if candidate.is_file():
                ...
                break
        raw_cfg = os.environ.get("RECALL_PY_CONFIG", "").strip()
        if raw_cfg:
            user = Path(raw_cfg).expanduser()
            if user.is_file():
                merged = _deep_merge(merged, _read_yaml(user))
        else:
            xdg = Path(os.environ.get("XDG_CONFIG_HOME", "~/.config")).expanduser() / "recall-py" / "config.yaml"
            if xdg.is_file():
