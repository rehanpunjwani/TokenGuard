...
from recall_py import engine
from recall_py.app import open_connection
from recall_py.indexing import index_workspace
from recall_py.metrics import summary as metrics_summary
from recall_py.ollama_client import OllamaClient
from recall_py.settings import AppSettings
from recall_py.threads import default_workspace_fingerprint

mcp = FastMCP(
    "RecallPy",
    instructions=(
        "RecallPy injects retrieved project context — do NOT ignore context_pack. "
        "On each user question: call recall_py_handle_query (returns context_pack.citations). "
        "Ground answers in those citations; read source files for code work. "
        "For complex tasks call recall_py_escalate_pack before deep reasoning. "
        "After your reply call recall_py_ingest_turn(role=assistant). "
        "Run recall_py_index once per repo if context is empty."
    ),
    log_level="ERROR",
)
...
def _bootstrap() -> tuple[AppSettings, OllamaClient]:
    ...
    try:
        if _settings is None:
            cfg = os.environ.get("RECALL_PY_CONFIG", "").strip()
            path = Path(cfg).expanduser() if cfg else None
            _settings = AppSettings.load(path if path and path.is_file() else None)
            print(f"RecallPy: Loaded config (Ollama: {_settings.ollama.base_url})", file=sys.stderr)
        ...
        if _conn is None:
            ...
            print(f"RecallPy: Database ready at {_settings.resolved_db_path()}", file=sys.stderr)

        assert _settings is not None and _ollama is not None and _conn is not None
        return _settings, _ollama
    except Exception as e:
        print(f"RecallPy FATAL: Bootstrap failed: {e}", file=sys.stderr)
        raise
...
def _error_json(tool: str, e: Exception) -> str:
    ...
    return json.dumps(
        {
            "error": str(e),
            "hint": "Check that Ollama is running and models are pulled. Run `recall-py doctor` for diagnostics.",
        },
        ensure_ascii=False,
    )
...
@mcp.tool()
async def recall_py_handle_query(
    query: str,
    ...
    try:
        ...
        return _error_json("recall_py_handle_query", e)


@mcp.tool()
async def recall_py_answer_stream(
    query: str,
    thread_id: str | None = None,
    workspace_fingerprint: str = "",
) -> str:
    ...
    try:
        ...
        return _error_json("recall_py_answer_stream", e)


@mcp.tool()
async def recall_py_answer(
    query: str,
    thread_id: str | None = None,
    workspace_fingerprint: str = "",
) -> str:
    """Retrieve context_pack + local hint. Prefer recall_py_handle_query for new questions."""
    try:
        settings, ollama = _bootstrap()
        conn = _connection()
        ...
        return _error_json("recall_py_answer", e)


@mcp.tool()
async def recall_py_ingest_turn(
    role: str,
    content: str,
    thread_id: str | None = None,
    title: str = "",
    ...
    try:
        ...
        return _error_json("recall_py_ingest_turn", e)


@mcp.tool()
async def recall_py_escalate_pack(
    query: str,
    thread_id: str | None = None,
    workspace_fingerprint: str = "",
) -> str:
    ...
    try:
        ...
        return _error_json("recall_py_escalate_pack", e)


@mcp.tool()
async def recall_py_index(
    workspace_fingerprint: str = "",
    force: bool = False,
) -> str:
    ...
    try:
        ...
        return _error_json("recall_py_index", e)


@mcp.tool()
async def recall_py_metrics(thread_id: str | None = None) -> str:
    ...
    try:
        ...
        return _error_json("recall_py_metrics", e)
