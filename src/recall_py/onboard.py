...
def _user_config_dir() -> Path:
    ...
    if xdg:
        return Path(xdg).expanduser() / "recall-py"
    return Path.home() / ".config" / "recall-py"


def _shipped_default_yaml() -> str:
    text = resources.files("recall_py").joinpath("config", "default.yaml").read_text(encoding="utf-8")
    header = "# RecallPy user config (created by `recall-py onboard`).\n\n"
    return header + text
...
def run(
    *,
    ...
    typer.secho("RecallPy onboarding", fg="cyan", bold=True)
    typer.echo("")

    raw = os.environ.get("RECALL_PY_CONFIG", "").strip()
    if raw:
        typer.echo(f"Using RECALL_PY_CONFIG={raw} (personal ~/.config file is ignored unless unset).")
        typer.echo("")
    ...
    if reachable:
        ...
        typer.secho(
            ...
            "Install from https://ollama.com and run `ollama serve`, then re-run onboard or `recall-py doctor`.",
            fg="yellow",
        )
    typer.echo("")
    ...
    mcp_command = shutil.which("recall-py") or "recall-py"

    typer.secho("Next steps", fg="cyan", bold=True)
    typer.echo("  1. HTTP API:     recall-py serve")
    typer.echo(
        "  2. MCP (IDE):    enable MCP + call `recall_py_handle_query` each turn "
    ...
    typer.echo("  6. Health check: recall-py doctor")
    typer.echo("")
    typer.echo("MCP snippet (copy into Cursor / Claude MCP settings):")
    if raw:
        mcp_env = {"RECALL_PY_CONFIG": str(Path(raw).expanduser().resolve())}
    elif user_yaml.is_file():
        mcp_env = {"RECALL_PY_CONFIG": str(user_yaml.resolve())}
    else:
        mcp_env = {}
    typer.echo(
        json.dumps(
    ...
    typer.echo("")
    if mcp_command == "recall-py":
        typer.echo(
            "Tip: If your IDE cannot find `recall-py` on PATH, use `python3.12` (or `python3`) "
            'with args ["-m","recall_py","mcp-stdio"] — see README “Cursor MCP config”.'
        )
        typer.echo("")
