...
from recall_py.engine import answer as engine_answer
...
from recall_py.store.db import connect, migrate
from recall_py.store.repository import proxy_cache_get, proxy_cache_put


def create_app(settings: AppSettings, conn: sqlite3.Connection) -> FastAPI:
    app = FastAPI(title="RecallPy", version="0.1.0")
            ...
            if settings.proxy.try_local_first:
                ...
                for m in reversed(messages):
                    if m.get("role") == "user":
                        ...
                        break
                if last_user.strip():
                    tid = request.headers.get("x-recall-py-thread-id")
                    ...
                    if draft.get("mode") == "draft" and draft.get("draft"):
                        ...
                        resp = openai_style_response(
                            body.get("model", "recall-py-local"),
                            draft["draft"],
                        )
