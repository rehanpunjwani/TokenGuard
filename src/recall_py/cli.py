...
from recall_py import onboard as onboard_mod
from recall_py.app import create_app, open_connection
from recall_py.hooks import run_after_response, run_before_prompt
from recall_py.indexing import index_workspace
from recall_py.mcp_server import run_mcp
...
app = typer.Typer(help="RecallPy: local cache, RAG, and optional OpenAI-compatible proxy.")
...
def metrics(
    ...
    typer.secho(f"RecallPy metrics ({scope})", fg="cyan", bold=True)
...
def index(
    workspace: Path | None = typer.Option(
        ...
        "--workspace",
        "-w",
        help="Repo root to index (default: cwd or RECALL_PY_WORKSPACE).",
    ),
...
def mcp_stdio(
    config: Path | None = typer.Option(
        ...
        dir_okay=False,
        readable=True,
        help="Optional YAML config; can also set RECALL_PY_CONFIG.",
    ),
) -> None:
    """Run MCP server on stdio (for Cursor, Claude Code, Claude Desktop)."""
    if config is not None:
        os.environ["RECALL_PY_CONFIG"] = str(config.expanduser().resolve())
    run_mcp()
